...
function chart_zoom(range_data, selection) {
    ...
    var margin = { top: 40, right: 30, bottom: 30, left: 30 },
        width = (range_data[1] range_data[1]) * 5 - margin.left - margin.right,
    ...
    filtered_data = _.filter(sort_Data, d => ((d.date >= sort_Data[range_data[0]].date) && (d.date <= sort_Data[range_data[1]].date))) console.log(filtered_data) var xScale = d3.scaleLinear().domain([-1, range_data[1]])
        .range([0, width])
