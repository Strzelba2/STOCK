...
function chart_zoom(range_data, selection) {
    ...
    var yScale_voume = d3.scaleLinear().domain([0, d3.max(filtered_data.map((x) => { return x.volume; }))]).range([100, 0]).nice();
    ...
    Volume
        ...
        .attr('x', (data, i) => xScale(i) - xBand.bandwidth())
        .attr("class", "volume")
        .attr('y', data => yScale_voume(data.volume))
        .attr('width', xBand.bandwidth())
        .attr('height', d => yScale_volume)
        .attr("fill", d => (d.open === d.close) ? "silver" : (d.open > d.close) ? "red" : "green")
