...
function chart_zoom(range_data, selection) {
    ...
    points.map((x) => {
            ...
            console.log(xScale_focus.invert(selection[0])]) console.log((x.scale[0] - selection[0])) var tradingLIne = chart.append("g")
            .attr("class", "tradingLIne")
        var line_trend = tradingLIne.append("line")
            .attr("id", `${ x.name }`)
            .attr("class", "drowLine")
            .attr("x1", xScale((x.scale[0] - selection[0])))
            .attr("y1", yScale(x.scale[1]))
            .attr("x2", xScale((x.scale[2] - selection[0])))
