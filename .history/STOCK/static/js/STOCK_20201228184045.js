...
function chart_zoom(range_data, selection) {
    console.log(xScale_focus.invert(selection[0]), xScale_focus.invert(selection[1]))
    var zoom_off = false
    ...
    var drow_line = chart.append("rect")
        .attr("id", "rect")
        .attr("width", width)
        .attr("height", height)
        .style("fill", "#E5E5E5")
    ...
    points.map((x) => {
        ...
        console.log(xScale_focus.invert(selection[0]))
        console.log((x.scale[0] - xScale_focus.invert(selection[0])))
        var tradingLIne = drow_line.append("g")
    ...
    function zoomed() {
        if (zoom_off === false) {
            ...
            var line_zoom = drow_line.selectAll(".drowLine")
            ...
            var Circle_zoom = drow_line.selectAll('circle')
    ...
    function zoomend() {
        if (zoom_off === false) {
            ...
            resizeTimer = setTimeout(function() {
                    ...
                    var line_zoom = dr.selectAll(".drowLine")
