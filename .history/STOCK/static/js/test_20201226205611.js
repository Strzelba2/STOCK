...
function chart_zoom(range_data, selection) {
    ...
    var margin = { top: 40, right: 30, bottom: 30, left: 30 },
        width = range_data[1] * 12 - margin.left - margin.right,
    ...
    var xScale = d3.scaleLinear().domain([-1, range_data[1]])
        .range([0, width])
    var xBand = d3.scaleBand().domain(d3.range(-1, range_data[1])).range([0, width]).padding(0.3)

    var xDateScale = d3.scaleQuantize().domain([0, range_data[1]]).range(filtered_data)
        ...
        if (zoom_off === false) {
            ...
            var line_zoom = chart.selectAll(".drowLine")
                .attr("x1", (line_zoom, x, arr) => {
                    ...
                    console.log(xScale.invert(arrpoints.xy[0]))
                    return xScaleZ(xScale.invert(arrpoints.xy[0]))
                })
                .attr("x2", (line_zoom, x, arr) => {
                    ...
                    console.log(xScale.invert(arrpoints.xy[2]))
                    return xScaleZ(xScale.invert(arrpoints.xy[2]))
                })
            console.log(line_zoom)
            console.log(line1)
...
var focus_chart = d3.select("#chart")
    .append("svg:svg")
    .attr("viewBox", [0, 0, width_focus, height_focus])
    .style("display", "block");
